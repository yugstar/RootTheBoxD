...
class UserFileUploadHandler(BaseHandler):
    ...
    def get(self, *args, **kwargs):
        if options.team_sharing:
            ...
            self.render(
                "file_upload/upload_files.html", errors=None, shares=user.team.files
            )
        else:
            self.redirect("/404")
            ...
            if hasattr(self.request, "files"):
                ...
                if not len(self.errors):
                    if user.is_admin():
                        ...
                        self.redirect("/user/uploads/files")
                else:
                    self.render(
                        "file_upload/upload_files.html",
                        errors=self.errors,
                        shares=shares,
                    )
            else:
