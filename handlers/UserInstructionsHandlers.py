...
class UserInstructionsHandlers(BaseHandler):

    """Handles file shares for users"""
    ...
    def get(self, *args, **kwargs):
        if options.team_sharing:
            ...
            self.render(
                "file_upload/upload_instructions.html", errors=None, shares=user.team.files
            )
        else:
            self.redirect("/404")
            ...
            if hasattr(self.request, "files"):
                ...
                if not len(self.errors):
                    if user.is_admin():
                        ...
                        self.redirect("/user/uploads/instructions")
                else:
                    self.render(
                        "file_upload/upload_instructions.html",
                        errors=self.errors,
                        shares=shares,
                    )
            else:
...
class UserFileDownloadHandler(BaseHandler):
...
class UserFileDeleteHandler(BaseHandler):
