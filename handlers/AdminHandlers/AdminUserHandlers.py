...
class AdminEditUsersHandler(BaseHandler):
    ...
    @authorized(ADMIN_PERMISSION)
    def post(self, *args, **kwargs):
        uri = {"user": self.edit_user, "team": self.edit_team, "task": self.edit_task}
    ...
    def edit_task(self):
        """Edits the task object"""
        try:
            task = Task.by_uuid(self.get_argument("uuid", ""))
            if task is None:
                raise ValidationError("Task does not exist")
            task.name = self.get_argument("name", task.name)
            task.description = self.get_argument("motto", task.description)
            if hasattr(self.request, "files") and "avatarfile" in self.request.files:
                task.avatar = self.request.files["avatarfile"][0]["body"]
            else:
                avatar = self.get_argument("avatar", task.avatar)
                if task.avatar != avatar and avatar != "":
                    # allow for default without setting
                    task._avatar = avatar
            ...
            self.render("/admin/view/managetask", errors=[str(error)])
...
class AdminDeleteUsersHandler(BaseHandler):
    ...
    @authorized(ADMIN_PERMISSION)
    def post(self, *args, **kwargs):
        uri = {"user": self.del_user, "team": self.del_team, "task": self.del_task}
        if len(args) and args[0] in uri:
            uri[args[0]]()
        else:
            self.redirect("/admin/users")
    ...
    def del_task(self):
        """
        Delete task objects in the database.
        """
        task = Task.by_uuid(self.get_argument("uuid", ""))
        if task is not None:
            logging.info("Deleted Task: '%s'" % str(task.name))
            self.dbsession.delete(task)
            self.dbsession.commit()
            self.redirect("/admin/view/managetask")
        else:
            self.render("admin/view/managetask.html", errors=["Task does not exist"])
