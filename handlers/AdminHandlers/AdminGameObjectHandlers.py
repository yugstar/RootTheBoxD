...
class AdminCreateHandler(BaseHandler):
    ...
    def post(self, *args, **kwargs):
        ...
        if len(args) and args[0] in game_objects:
            ...
            self.render("public/404.html")
# for task function same as create team --test start
    def create_task(self):
        """Admins can create tasks manually"""
        try:
            name = self.get_argument("task_name", "")
            if Task.by_name(name) is not None:
                raise ValidationError("Task already exists")
            task = Task()
            task.name = self.get_argument("task_name", "")
            task.description = self.get_argument("description", "")
            # if not self.config.banking:
            #     task.set_score("start", 0)
            # else:
            #     task.set_score("start", options.starting_task_money)
            # level_0 = GameLevel.by_number(0)
            # if not level_0:
            #     level_0 = GameLevel.all()[0]
            # task.game_levels.append(level_0)
            self.dbsession.add(task)
            self.dbsession.commit()

            # Avatar
            avatar_select = self.get_argument("task_avatar_select", "")
            if avatar_select and len(avatar_select) > 0:
                task._avatar = avatar_select
            elif hasattr(self.request, "files") and "avatar" in self.request.files:
                task.avatar = self.request.files["avatar"][0]["body"]
            self.dbsession.add(task)
            self.dbsession.commit()
            self.event_manager.push_score_update()
            self.redirect("/admin/view/managetask")
        except ValidationError as error:
            self.render("admin/create/task.html", errors=[str(error)])
...
class AdminViewHandler(BaseHandler):
    ...
    def get(self, *args, **kwargs):
        ...
        uri = {
            ...
            "managetask": "admin/view/managetask.html",
